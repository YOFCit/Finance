...
class UrgentPaymentRequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            Mail::to($this->mail)->send(new NewRequestMail($urgentRequest, $link));
        } catch (\Exception $e) {
            dd($e->getMessage());
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'requestor' => 'required|string|min:1',
            'department' => 'required|string|min:1',
            'expense_no' => 'nullable|string|unique:urgent_payment_requests,expense_no,' . $id,
        ...
        $requestModel->update([
            ...
            'risk' => $validated['risk'] ?? null,
            'causing_department' => $validated['causing_department'] ?? null,
            'signature_path' => !empty($validated['signature']) ? $this->saveSignature($validated['signature']) : $requestModel->signature_path,
        ]);
    ...
    public function getName(Request $request)
    {
        ...
        return response()->json([
            'nombre' => $empleado->name ?? 'Not found',
            'departamento' => $empleado->department ?? 'Not found',
            'causing_department' => $empleado->causing_department ?? 'Not found'
        ]);
    }
    ...
    public function approveurl($token)
    {
        try {
            ...
            return redirect()->route('home')->with('error', 'Request not found or invalid token.');
        }
    }
    ...
    public function updateApprovalStatus(Request $request, $id)
    {
        ...
        if (!in_array($status, ['Approve', 'Reject'])) {
            return redirect()->back()->with('error', 'Invalid status. Only Approve or Reject allowed.');
        }
        ...
        return redirect()->back()->with('success', "Request status updated to: $status.");
    }
